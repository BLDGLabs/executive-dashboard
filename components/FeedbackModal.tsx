...
export default function FeedbackModal({ apiUrl, onClose }: FeedbackModalProps) {
  ...
  const [submittedBy, setSubmittedBy] = useState<string>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("exec-dashboard-name") || "";
    }
    return "";
  });
  ...
  const handleSetName = () => {
    ...
    localStorage.setItem("exec-dashboard-name", name);
    setSubmittedBy(name);
    setNameSet(true);
  };
  ...
  }, [input, isTyping, messages, submittedBy, feedbackId, apiUrl]);
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
        ...
        {!nameSet ? (
          <div className="p-6 flex flex-col gap-4">
            <p className="text-gray-700 dark:text-gray-300 text-sm">Before we begin, what's your name?</p>
            <input
              type="text"
              value={nameInput}
              ...
              className="border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              autoFocus
            />
            <button
